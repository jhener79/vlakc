		...
		if(!empty($this->rows)){
			...
			<?php foreach($this->rows as $row){
				...
				<li class="hikashop_product_list_item" <?php echo $width; ?>>
						...
						<a href="<?php echo $link;?>" title="<?php echo $this->escape($this->row->product_name); ?>" class="uk-align-left">
							<?php 
								$image_options = array('default' => true,'forcesize'=>$this->config->get('image_force_size',true),'scale'=>$this->config->get('image_scale_mode','inside'));
								$img = $this->image->getThumbnail(@$this->row->file_path, array('width' => '80', 'height' => '100'), $image_options);
								if($img->success) {
