jQuery(document).ready(function($) {
	...
	$choice.on('click', function(e) {
		...
		shortcodea = $(this).data('shortcode');
...
//                     if(showShortcode != undefined && showShortcode == 2){
//			update_preview();
//                    }else{
//                        show_shortcode();
//                    }
...
function bindOnChange(){    
	...
	function add_preset(id, name) {
		...
		su_ap = localStorage.getItem(shortcodea);
		
		if(su_ap){
                su_ap = jQuery.parseJSON(su_ap)
		}else{
		 var su_ap = {};
		}
	...
	function update_preview(forced) {
        $button = $('#su-generator .su-generator-toggle-preview')
	...
	function show_shortcode(forced) {
        $button = $('#su-generator .su-generator-shortcode-preview')
